...
benchmark_runner = File.read(File.join(root, "tools/commands/run/writing-benchmark.sh"))
...
%w[candidate_sha dataset_sha256 policy_sha256 results_sha256 runner_sha256].each do |marker|
  errors << "benchmark runner evidence envelope missing #{marker}" unless benchmark_runner.include?(marker)
end

acceptance_runner = File.read(File.join(root, "tools/commands/run/p0-acceptance.sh"))
